...
3. "channel_proof_1" — one sentence, 15-25 words, introducing the channel via its scale. 1st channel of a niche opens with "This channel...". Rotate openers across niches; never use "This channel..." for two niches in a row within any 3-niche stretch. Use the subscriber count and/or age and/or video_count. PREFER to lead with a 3-6 word gist of WHAT they make before the scale (recipe-first hook), e.g. "This channel ranks fictional monsters by size — and already has 99,000 subscribers." (the winner format hooks with the format, then the proof).
...
function fmtMoneyForPrompt(s: ChannelSlots): string {
  ...
  // NB: deliberately NOT exposing yearly/monthly headline here — money_math
  // stops at the unimpeachable top-video lump sum (no wide annual range).
