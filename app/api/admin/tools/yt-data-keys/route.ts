...
import {
  startYtDataKeyImport,
  getYtDataKeyImportState,
  DEFAULT_YT_DATA_KEY_JOB_ID,
} from '@/lib/yt-data-key-import';
 ...
 *
 * The job pulls tasks awaiting employer review from the xgodo YT Data
 * API key job, tests each candidate key via residential proxy against
 * youtube/v3/i18nLanguages, inserts the good ones into xgodo_api_keys
 * (service='youtube_data'), and confirms / declines each xgodo task
 * accordingly.
 */

function publicState() {
  const s = getYtDataKeyImportState();
  return {
    running: s.running,
    jobKey: s.jobKey,
    ...
    defaultJobId: DEFAULT_YT_DATA_KEY_JOB_ID,
  };
}
