...
/**
 * GET /api/admin/tools/ai-studio-keys/export
 *
 * Dumps all Google AI Studio keys from xgodo_api_keys as a browser
 * download. Useful for piping into other systems, ops audits, or
 * yanking the inventory out before a migration.
 *
 * Query params:
...
export async function GET(req: NextRequest) {
  ...
  const conds: string[] = [`service = 'google_ai_studio'`];
  const args: (string)[] = [];
  let p = 1;
  if (status !== 'all') {
    conds.push(`status = $${p++}`);
  ...
  const base = `ai-studio-keys_${status}${source ? '_' + source : ''}_${stamp}`;
