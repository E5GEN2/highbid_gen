...
export function ChannelAgeChip({ createdAt, firstUploadAt, dormancyDays, agedThresholdDays = 365 }: ChannelAgeProps) {
  ...
  // Nothing to show if we don't have at least one of the dates
  if (!created && !firstUp) return null;

  // Prefer active age. Fall back to creation age when first-upload hasn't been
  // detected yet (channel with >200 uploads, or age check hasn't run).
