...
export default function NicheInsights() {
  ...
  return (
    <div className="px-8 py-8 max-w-7xl mx-auto space-y-6">
      {/* Opportunity Indicators — 4-card row ABOVE scatter */}
      {distLoading ? (
        <OpportunityIndicatorsSkeleton />
      ) : scatterDots.length > 0 ? (
        <OpportunityIndicators dots={scatterDots} />
      ) : null}
...
/* ─────────────────────────────────────────────────────────────
   Opportunity Indicators — NOS, Top-Left Density, Newcomer Rate, Low-Sub Ceiling
   Computed client-side from scatter dots (no extra API call needed)
   ───────────────────────────────────────────────────────────── */

interface IndicatorDot { s: number; v: number; a: number | null; }
...
function computeIndicators(dots: IndicatorDot[]) {
  if (dots.length === 0) {
  ...
  // (Age data can be null — we only use dots with known age)
...
function OpportunityIndicators({ dots }: { dots: IndicatorDot[] }) {
      ...
      <IndicatorCard
        ...
        tooltip={
          <>
            ...
            <div>For each video we compute <code className="text-amber-400">log(views) / log(subs)</code> and take the median across the niche. Higher = views outpace subs, meaning the algorithm pushes content from small creators.</div>
      ...
      <IndicatorCard
        ...
        tooltip={
          <>
            ...
            <div>Counts videos with above-median views AND below-median subs — the top-left quadrant of the scatter. A high % means the pattern is common, not just a single lucky outlier.</div>
            <div className="mt-2 space-y-0.5">
      ...
      <IndicatorCard
        ...
        tooltip={
          <>
            ...
            <div>Median views of channels under 6 months old, divided by the niche&apos;s overall median. 100% means newcomers land in the same ballpark as veterans.</div>
            <div className="mt-2 space-y-0.5">
      ...
      <IndicatorCard
        ...
        tooltip={
          <>
            ...
            <div>Top 10% of view counts among videos from channels with under 10K subscribers. Shows the realistic upside of a single video while you&apos;re still small.</div>
            <div className="mt-2 space-y-0.5">
...
function OpportunityIndicatorsSkeleton() {
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
      {[0, 1, 2, 3].map(i => (
