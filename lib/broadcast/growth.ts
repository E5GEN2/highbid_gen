...
import { gatherChannel, sendRichChannelPost, isAutoGenerated, startedPostingLine, accountCreatedLine, NOT_AUTOGEN_SQL, type ChannelPostConfig, type ChannelRow, type VideoRow } from './spotlight';

interface GrowthConfig extends ChannelPostConfig { perTick: number; cooldownDays: number; minMult: number; minSubs0: number; maxPerDay: number; minGapMinutes: number }
...
async function loadConfig(pool: Pool): Promise<GrowthConfig> {
  const r = await pool.query<{ key: string; value: string }>(
    `SELECT key, value FROM admin_config WHERE key IN
       ('broadcast_telegram_token','broadcast_telegram_chat','broadcast_growth_per_tick',
        ...
        'broadcast_growth_max_per_day','broadcast_growth_min_gap_minutes')`,
  );
  ...
  return {
    ...
    // ~2.5h: with maxPerDay=6 this spreads stories across the waking day.
    minGapMinutes: parseInt(c.broadcast_growth_min_gap_minutes) || 150,
  };
}
...
export function buildGrowthCaption(ch: ChannelRow, vids: VideoRow[], gd: GrowthData, format = '', firstPostedAt: Date | null = null): string {
...
export async function sendGrowthFor(
  ...
  const caption = buildGrowthCaption(g.ch, g.vids, gd, g.format, g.firstPostedAt);
...
export async function runGrowthStoryTick(): Promise<GrowthTickResult> {
  ...
  // SPACING: the daily cap alone would fire the whole day's allowance in the
  // first N ticks (N posts in N minutes, then 24h of silence) — a bad rhythm for
  // a public channel. Enforce a minimum gap so stories trickle across the day.
  const last = await pool.query<{ mins: string | null }>(
    `SELECT EXTRACT(EPOCH FROM (NOW() - MAX(posted_at)))/60 AS mins
       FROM broadcast_posts WHERE kind = 'growth_story' AND ok`);
  const sinceLast = last.rows[0]?.mins != null ? parseFloat(last.rows[0].mins) : Infinity;
  if (sinceLast < cfg.minGapMinutes) {
    return { ran: true, reason: `spacing (${Math.round(sinceLast)}m < ${cfg.minGapMinutes}m)`, sent: 0 };
  }
