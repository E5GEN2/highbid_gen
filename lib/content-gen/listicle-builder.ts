...
export async function buildListicleScript(opts: BuildListicleOpts): Promise<BuildListicleResult> {
  ...
  for (let i = 0; i < channels.length; i++) {
    ...
    // (transcript-grounded, generated by the analysis pipeline). Diegetic:
    // the clip's own audio sits at ~-15dB under the narrator.
    const recipeSlots: Slot[] = vars.recipe_beats
      ...
      .map((b, bi) => ({
        ...
        compose: {
          ...
          hold_s: '{{narr.duration_s}}',
          layers: [
            { from: 'main', channel: 'video' as const, fit: 'contain' as const, ken_burns: 'none' as const, player_frame: true, diegetic: true },
