...
function analyzeFrame(filename: string): FrameData {
...
function calculateBounds(region: Array<{x: number, y: number}>): PanelBounds {
...
function calculateCentroid(region: Array<{x: number, y: number}>): {x: number, y: number} {
...
function calculateEdgeTouch(bounds: PanelBounds, canvasWidth: number, canvasHeight: number) {
...
function inferGridSize(panels: Panel[], width: number, height: number): string {
  if (panels.length === 1) return '1x1';
  if (panels.length === 2) {
  ...
  if (panels.length === 3) return 'custom';
  if (panels.length === 4) {
    ...
    centroids.forEach((c) => {
      if (c.x < avgX && c.y < avgY) topLeft++;
      else if (c.x >= avgX && c.y < avgY) topRight++;
      else if (c.x < avgX && c.y >= avgY) bottomLeft++;
      else bottomRight++;
    });
...
export async function scanFrames(): Promise<void> {
...
export function getFrameManifest(): Manifest {
  try {
