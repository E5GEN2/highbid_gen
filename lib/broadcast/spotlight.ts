...
export interface VideoRow { title: string | null; thumbnail: string | null; view_count: number | null; url: string | null; posted_at: Date | null; duration_seconds: number | null }
...
export async function gatherChannel(pool: Pool, channelId: string): Promise<GatheredChannel | null> {
  ...
  const vidsRes = await pool.query<VideoRow>(
    `SELECT title, thumbnail, view_count, url, posted_at, duration_seconds FROM niche_spy_videos
  ...
  // Durations, durably: STORED values first (fetched once = known forever), then
  // one API round only for the ids still missing, then write those back so the
  // next post for this channel needs no API at all. The live fetch failing can
  // no longer strip badges/format off a post whose durations were ever resolved.
  const durations = new Map<string, number>();
  const missing: string[] = [];
  for (const v of vidsRes.rows) {
    const ytId = extractYtId(v.url);
    if (!ytId) continue;
    if (v.duration_seconds != null && v.duration_seconds > 0) durations.set(ytId, v.duration_seconds);
    else missing.push(ytId);
  }
  if (missing.length) {
    const fetched = await fetchDurations(missing);
    for (const [id, sec] of fetched) durations.set(id, sec);
    // Persist (best-effort, channel-scoped; urls are youtu.be/<id> or contain the id).
    for (const [id, sec] of fetched) {
      await pool.query(
        `UPDATE niche_spy_videos SET duration_seconds = $1 WHERE channel_id = $2 AND url LIKE '%' || $3 || '%'`,
        [sec, channelId, id],
      ).catch(() => {});
    }
  }
