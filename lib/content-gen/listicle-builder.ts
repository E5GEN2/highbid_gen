...
export interface BuildListicleOpts {
  ...
  /** Operator overrides for conditional beats (per-beat on/off/auto +
   *  threshold tweaks). Omitted/auto = current behavior. */
  flags?: BeatFlags;
}
...
export interface BuildListicleResult {
  ...
  /** Per-channel conditional-beat decisions + signals (for the pre-render
   *  summary). Recorded additively; does not affect emission. */
  beatPlan: ChannelBeatPlan[];
  error?: string;
}

export async function buildListicleScript(opts: BuildListicleOpts): Promise<BuildListicleResult> {
  const beat_id = opts.beat_id;
  const flags: BeatFlags = opts.flags ?? {};
  const channels = opts.channels.slice(0, 16);
  const allSlots: ConcreteScript['slots'] = [];
  const beatPlans: ChannelBeatPlan[] = [];
  ...
  for (let i = 0; i < channels.length; i++) {
    ...
    const tinyCatalog = ch.video_count != null && ch.video_count > 0 && ch.video_count <= (flags.videoBoxMaxVideos ?? 12);
    const videoCountHook = applyToggle(!!(tinyCatalog && strong), flags.videoCountBox);
    if (videoCountHook) {
    ...
    const moneyMath = applyToggle(true, flags.moneyMath)
      ? buildMoneyMathSlots(niche_index, ch, {
    ...
    const recipeSlots: Slot[] = (applyToggle(true, flags.recipe) ? vars.recipe_beats : [])
    ...
    const CALLOUT_OUTLIER_MULT = flags.calloutOutlierMult ?? 8;  // top ≥ N× median = standout
    const PANO_MIN_VIEWS = flags.panoMinViews ?? 50_000;         // per-video floor ("xxk each")
    const calloutWorthy = applyToggle(medianViews > 0 && topViews >= CALLOUT_OUTLIER_MULT * medianViews, flags.callout);
    const panoWorthy = applyToggle(sortedViews.length >= 6 && p10Views >= PANO_MIN_VIEWS, flags.pano);
    ...
    const ageWorthy = applyToggle(vars.age_months != null && vars.age_months <= (flags.ageMaxMonths ?? 4), flags.ageCard);
    const ageCardSlot = (ageWorthy && ageNarration && ageFragment)
      ? {
      ...
      : null;
    const emphasisSlot = (applyToggle(!!emphasisLine, flags.emphasis) && emphasisLine)
      ? makeFramingSlot(`niche_${niche_index}_emphasis_card`, 'emphasis_card', emphasisLine,
          ...
          ['whoosh'])
      : null;
    const rapidFireSlots = applyToggle(true, flags.rapid)
      ? await buildTopViewsRapidFireSlots(niche_index, ch, {
    ...
    // Operator suppression of the channel-B / saturation blocks (flags). They
    // still compute above (safest — no change to that logic); we just drop the
    // emitted slots when forced off.
    if (flags.channelB === 'off') channelBSlots = [];
    if (flags.saturation === 'off') saturationSlots = [];
    ...
    // Record the conditional-beat plan for this channel — reads the emitted
    // beat_ids + the gate signals already computed above. Purely observational.
    const has = (bid: string) => nicheGroup.some(s => s.beat_id === bid);
    const hv = (n: number) => n >= 1e6 ? `${(n / 1e6).toFixed(1)}M` : n >= 1e3 ? `${Math.round(n / 1e3)}K` : String(Math.round(n));
    const rapidCount = nicheGroup.filter(s => s.beat_id === 'top_views_rapid').length;
    const recipeCount = nicheGroup.filter(s => s.beat_id === 'recipe_demo').length;
    beatPlans.push({
      channelId: ch.channelId,
      niche_index,
      channelLabel: ch.channel_name ?? ch.channelId,
      signals: {
        topViews, medianViews, p10Views, recentN: sortedViews.length,
        ageMonths: vars.age_months ?? null, videoCount: ch.video_count ?? null,
        subs: ch.subscriber_count ?? null, totalViews: ch.total_views ?? null,
      },
      beats: {
        top_views_rapid:   { fire: rapidCount > 0, reason: `${rapidCount} top-video card(s)` },
        top_video_callout: { fire: has('top_video_callout'), reason: calloutWorthy ? `breakout: top ${hv(topViews)} ≥ ${CALLOUT_OUTLIER_MULT}× median ${hv(medianViews)}` : `top ${hv(topViews)} < ${CALLOUT_OUTLIER_MULT}× median ${hv(medianViews)}` },
        top_videos_pano:   { fire: has('top_videos_pano'), reason: panoWorthy ? `floor: p10 ${hv(p10Views)} ≥ ${hv(PANO_MIN_VIEWS)} (n=${sortedViews.length})` : `p10 ${hv(p10Views)} < ${hv(PANO_MIN_VIEWS)} or n<6` },
        channel_age_card:  { fire: has('channel_age_card'), reason: vars.age_months != null ? `posting ${vars.age_months}mo (≤4 fires)` : 'no posting-age data' },
        video_count_box:   { fire: videoCountHook, reason: videoCountHook ? `${ch.video_count} videos + strong result` : `${ch.video_count ?? '?'} videos / not a small-catalog hook` },
        money_math:        { fire: has('money_math'), reason: (ch.top_video_view_count ?? 0) >= 1000 ? 'monetizable' : 'top video < 1000 views → skip' },
        recipe_demo:       { fire: recipeCount > 0, reason: `${recipeCount} recipe clip(s)` },
        channel_b:         { fire: has('channel_b_proof'), reason: has('channel_b_proof') ? 'qualified channel B found' : 'no qualified B' },
        saturation:        { fire: has('saturation_callout'), reason: has('saturation_callout') ? 'saturated cluster' : 'not saturated' },
        emphasis_card:     { fire: has('emphasis_card'), reason: has('emphasis_card') ? 'bank line present' : 'no bank line' },
      },
    });
  }

  if (acceptedCount === 0) {
    return { script: null, channelEvents, failures, beatPlan: beatPlans, error: 'every channel failed to author' };
  }
  ...
  return { script, channelEvents, failures, beatPlan: beatPlans };
}
